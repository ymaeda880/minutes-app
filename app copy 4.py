...
pg = st.navigation(
    {
        "議事録": [
            #st.Page("pages/05_———（議事録）———.py", title="議事録"),
            st.Page("pages/07_議事録作成（新）.py", title="議事録作成（新）"),
        ],
        "逐語録までの処理": [
            #st.Page("pages/10_———（逐語録までの処理）———.py", title="逐語録までの処理"),
            st.Page("pages/20_音声ファイル分割_ストレージ対応.py", title="音声ファイル分割"),
            st.Page("pages/21_文字起こし_ストレージ対応.py", title="文字起こし"),
            st.Page("pages/22_話者分離_ストレージ対応.py", title="話者分離"),
            st.Page("pages/30_重複箇所検出_ストレージ対応.py", title="重複箇所検出"),
        ],
        "旧ページ": [
            #st.Page("pages/60_———（OLD）———.py", title="OLD"),
            st.Page("pages/61_音声ファイル分割.py", title="音声ファイル分割（旧）"),
            #st.Page("pages/62_文字起こし（Gemini）（連続対応）.py", title="文字起こし Gemini"),
            st.Page("pages/63_文字起こし（連続対応）.py", title="文字起こし 連続対応"),
            st.Page("pages/65_重複箇所検出.py", title="重複箇所検出（旧）"),
            st.Page("pages/67_話者分離.py", title="話者分離（旧）"),
            st.Page("pages/68_話者分離（Gemini）.py", title="話者分離 Gemini"),
        ],
        "その他": [
            #st.Page("pages/48_———.py", title="その他"),
            st.Page("pages/50_ポータルへ戻る.py", title="ポータルへ戻る"),
            #st.Page("pages/52_——————————.py", title="区切り"),
            st.Page("pages/80_議事録ポータル.py", title="議事録ポータル"),
            st.Page("pages/100_ログインテスト.py", title="ログインテスト"),
            st.Page("pages/101_ログインテスト2.py", title="ログインテスト2"),
        ],
    }
)
