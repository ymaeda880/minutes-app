# -*- coding: utf-8 -*-
# lib/explanation.py
#
# 音声分割ツール（MP3/WAV）のロジックと使い方を説明する expander（Streamlit用）

from __future__ import annotations
import streamlit as st

# ============================================================
# imports
# ============================================================
import re
from contextlib import contextmanager

from common_lib.ui.banner_lines import render_banner_line_by_key
from common_lib.ui.theme_colors import get_theme_colors_from_banner_key
from common_lib.ui.expander_style import render_theme_expander_css

# ============================================================
# theme expander helper
# ============================================================
@contextmanager
def themed_expander(
    *,
    expander_id: str,
    title: str,
    expanded: bool = False,
):

    # ------------------------------------------------------------
    # theme
    # ------------------------------------------------------------
    banner_key = "light_green"

    theme = get_theme_colors_from_banner_key(
        banner_key
    )

    # ------------------------------------------------------------
    # safe key
    # ------------------------------------------------------------
    safe_key = re.sub(
        r"[^0-9a-zA-Z_]+",
        "_",
        expander_id,
    )

    k_expander = f"{safe_key}_theme_expander"

    # ------------------------------------------------------------
    # css
    # ------------------------------------------------------------
    render_theme_expander_css(
        container_key=k_expander,
        theme=theme,
    )

    # ------------------------------------------------------------
    # container
    # ------------------------------------------------------------
    theme_area = st.container(
        key=k_expander,
    )

    # ------------------------------------------------------------
    # expander
    # ------------------------------------------------------------
    with theme_area:
        with st.expander(
            title,
            expanded=expanded,
        ):
            yield

##########################
#
# 音声ファイル分解
#
###########################

def render_audio_split_expander() -> None:
    """
    音声分割ツール（MP3/WAV・オーバーラップ付き）のロジックと使い方を expander で描画します。
    pages/xx_音声分割.py などから呼び出して使います。
    """
    #with st.expander("ℹ️ 「音声ファイル分割」の使い方（クリックで展開）", expanded=False):
    with themed_expander(
        expander_id="audio_split",
        title="ℹ️ 音声分割ツールのしくみと使い方（クリックで展開）",
        expanded=False,
    ):

        st.markdown(
            """

            ## 1. 使用法

            1. 音声ファイルをアップロードします．「Drag and drop」にファイルをドラッグするか，
            「Browse files」からパソコン上のファイルを読み込んでください．

            2. ファイルが読み込まれると，自動的に音声ファイルの解析がおこなれます．

            3. 音声分割が終わると，「分割済み音声を生成（ZIP作成＋ストレージ保存）」のボタンが表示されます．
            このボタンを押すとファイル分割の処理が行われます．

            4. 処理が終わると，「分割ファイルをストレージに保存しました」というメッセージが出ます．
            この時点で，サーバーの内部ストレージに，分割ファイルが保存されています．

            5. 分割したファイルを自分のパソコンに保存したい場合には，「分割済み音声をZIPでダウンロード」ボタンを押してください．
            パソコンに分割したファイルを保存する必要がない時は，このボタンを押す必要はありません．
            以後の処理はサーバーの内部ストレージに保存したファイルから行うことができます．


            ## 2. このツールが行う処理（全体の流れ）

            1. アップロードした **MP3 / WAV** を読み込みます。  
            2. 指定した **チャンク長（分）** で音声を分割します。  
            3. 隣接チャンクには、指定した **オーバーラップ（重なり）** を付けます。  
            4. 各チャンクには開始時刻・終了時刻・持続時間を付与します。  
            5. 指定フォーマット（MP3 / WAV）でファイルを書き出し、ZIP にまとめます。  
            6. 分割一覧（プレビュー表）と ZIP ダウンロードボタンを表示します。

            <span style="color:red;">
            左側（サイドバー）の設定は，基本的にデフォルトのままで構いません．
            </span>

            ---

            ## 3. パラメータの意味

            ### 🔹 チャンク長（分）
            - 1チャンクあたりの再生時間です。
            - 例：15分 → 0–15分, 14分–29分, 28分–43分 … のように切り分けます。

            ### 🔹 オーバーラップ（分）
            - 隣り合うチャンクの重複部分。
            - 文字起こしや話者分離で文脈を保つために有効。
            - 例：チャンク長15分・オーバーラップ1分  

            > ⚠ **オーバーラップはチャンク長より短い必要があります。**

            ### 🔹 書き出しフォーマット（mp3 / wav）
            - WAV：PCM16 で出力  
            - MP3：選択したビットレートで出力（または「自動」）

            ### 🔹 ビットレート（MP3のみ）
            - MP3 の品質設定。（128k〜320k）

            ### 🔹 フェード（ms）
            - クリックノイズ低減のため、各チャンクの頭と末尾に
              フェードイン／フェードアウトを適用できます。

            ### 🔹 最後の“短すぎる尻尾”の吸収
            - 最後の端数部分が **オーバーラップより小さい場合**  
              → 無理に独立チャンクにせず **前のチャンクに吸収** します。
            - 不要な細切れファイルを防ぐオプションです。

            ---

            ## 4. 出力されるファイル

            ZIP の中には以下のファイルが格納されます．

            - `audio_part000_000000-001500.mp3` などのチャンク音声
            - 全チャンクのインデックス CSV  
              （開始ms / 終了ms / hh:mm:ss 付き）

            ---

            ## 5. 注意事項（保存時の警告について）

            LAN 内や自己署名 HTTPS 環境では、  
            **Chrome が ZIP ダウンロードを「安全でない」と警告する** 場合があります。

            - その場合は右上に出る警告 → **「保存」** をクリックしてください。
            - 内容はすべてローカル作成のファイルで安全です。

            ---

            ## 6. よくある質問（FAQ）

            ### Q1. 文字起こし用に最適な設定は？
            - **チャンク長：10〜20分**
            - **オーバーラップ：30秒〜1分**
            - Whisper / Deepgram 等との相性が良いです。

            ### Q2. MP3 と WAV のどちらが良い？
            - **WAV（PCM16）：** 高品質。文字起こし精度が上がりやすい  
            - **MP3：** 軽量で共有しやすい

            <span style="color:red;">
            通常は処理を軽くするためにMP3を使用してください．
            </span>

            <div style='height:10px;'></div>

            ### Q3. オーバーラップを付ける理由は？
            - 文脈が途切れにくくなるため  
            - AI 文字起こしが途中で発話を切り捨てる問題の対策

            """,
            unsafe_allow_html=True,
        )

###############
#
# 文字起こし
#
################

def render_transcribe_continuous_expander() -> None:
    """
    「文字起こし（ストレージ対応）」ページのロジックと使い方を expander で描画します。
    pages/05_文字起こし（連続対応）.py などから呼び出して使います。
    """
    with st.expander("ℹ️ 文字起こし（ストレージ対応）のしくみと使い方（クリックで展開）", expanded=False):
        st.markdown(
            """
            ## 1. 使い方

            1. 音声ファイルをアップロードします．通常は，「音声ファイル分割」によりサーバー内部に保存されたフィルを使用します．
            「入力方式」で「既存ジョブ」を選択し，「ジョブ選択」から「ジョブ（分割された音声ファイルの集合体）」を選択します．

            2. 「文字起こしを実行＋ストレージ保存」のボタンを押すと，ファイルの集合体に，順番に文字起こしが行われます．
            最後にそれぞれのファイルの文字起こしが結合されます．

            3. 文字起こしがすべて終わると，「ストレージに保存しました」というメッセージが出ます．
            このときに，すべてのファイル文字起こしをした結果が，分割されたままのテキストで，
            サーバー内部のストレージに保存されます．

            4. 「連結テキストをパソコンにダウンロード」ボタンを押すと，文字起こししたテキストファイルが１つのファイルの連結されて，自分のパソコンに
            ダウンロードできます．


            ## 2. このページの役割

            - OpenAI の **Transcribe / Whisper API** を使って、音声ファイルを文字起こしするページです。
            - **複数の音声ファイル** をアップロードすると、
              1ファイルずつ順番に文字起こしし、最後に **1つのテキストとして連結** します。
            - 連結テキストは、次のタブ「話者分離」へそのまま引き継げます。

            ---

            ## 3. 全体の処理の流れ

            1. API キーや価格テーブルなどの共通設定を読み込みます。
            2. サイドバーで **モデル・言語・プロンプト** 等を指定します。
            3. 複数の音声ファイルをアップロードします（順番通りに処理）。
            4. 各ファイルについて Transcribe / Whisper API を呼び出します（リトライ付き）。
            5. ファイルごとに
               - テキスト表示
               - 個別ダウンロード（.txt）
               - 推定コストと処理時間（メトリクス表）
               を表示します。
            6. 最後に、全てのテキストを **つなぎ目マーカー入り** で連結して表示・ダウンロード可能にします。
            7. 連結テキストをサーバー内部に保存し、
               話者分離タブへ引き継ぐためのボタンを表示します。

            ---

            ## 4. 主なパラメータの説明

            ### 🔹 モデル選択（whisper-1 / gpt-4o-mini-transcribe / gpt-4o-transcribe）
            - **whisper-1**  
              - 既存の Whisper API。互換性重視。（通常はこれを使用してください．）
            - **gpt-4o-mini-transcribe**  
              - 低コスト・高速。日常的な会議やメモ用途に向く。
            - **gpt-4o-transcribe**  
              - 精度重視。重要な議事録や長時間のインタビューなどに向く。


            ### 🔹 アップロードファイル
            - 複数ファイルを選択可能（.wav / .mp3 / .m4a / .webm / .ogg）。
            - アップローダに表示された順番のまま上から処理します。

            ### 🔹 言語コード（language）
            - 例：`ja`（日本語）, `en` （英語）など。  
            - 空欄にすると **自動判定** に任せます。


            ### 🔹 【…】除去オプション
            - `書き起こし後に【…】を除去する` にチェックを入れると、
              `strip_bracket_tags()` で全角の【…】に囲まれた短いタグを削除します。
            - 例：`【笑】`, `【ノイズ】` のようなラベルを後処理で消したい場合に使います。

            ---

            ## 5. 出力内容

            ### 5-1. ファイル別の表示

            各ファイルごとに、右側ペインに以下を表示します。

            - テキストエリア（個別の文字起こし結果）
            - `.txt` ダウンロードボタン（UTF-8）
            - メトリクス表
              - 処理時間（秒）
              - 音声長（秒 / 分）
              - 概算料金（USD / JPY）
              - request-id（トラブルシュート用）
              - 使用モデル

            ### 5-2. 連結テキスト

            全てのファイルを処理した後に：

            - 「🔗 連結テキスト（全ファイル）」として一括表示。
            - `.txt` で一括ダウンロード可能。
            - 「📋 連結テキストをコピー」ボタンで
              クリップボードへコピーするための小さな JavaScript ボタンを表示。
            - 各ファイルの境界には  
              `----- ここがつなぎ目です（◯◯ と次のファイルの間）-----`  
              というマーカーを自動挿入します。

            ### 5-3. 集計メトリクス

            - 全ファイルの
              - 合計処理時間
              - 合計音声長
              - 合計概算料金（USD/JPY）
            - さらにファイル別のサマリー表も `st.dataframe` で表示します。


            """
        )

##############
#
# 話者分離
#
###############

def render_speaker_prep_expander() -> None:
    """
    「話者分離」ページのロジックと使い方を expander で描画します。
    pages/xx_話者分離.py などから呼び出して使います。
    """
    with st.expander(
        "ℹ️ 「話者分離」のしくみと使い方（クリックで展開）",
        expanded=False,
    ):
        st.markdown(
            """

            ## 1. 使い方

            1. 文字起こし後のテキストファイルをアップロードします．通常は，「文字起こし」によりサーバー内部に保存されたフィルを使用します．
            「入力方式」で「既存ジョブ」を選択し，「ジョブ選択」から「ジョブ」を選択します．

            2. 左パネル（サイドバー）の「モデル設定」と「プロンプト設定」は，通常はそのままデフォルトままで構いません．

            3. 「文字起こしテキストを順番に話者分離（ストレージ保存＋連結）」のボタンを押すと，文字起こしテキストの集合体に，順番に話者分離が行われます．
            最後にそれぞれのファイルの話者分離後のテキストが結合されます．

            4. 話者分離がすべて終わると，「ストレージに保存しました」というメッセージが出ます．
            このときに，すべてのファイル話者分離を行った結果が，連結されて，
            サーバー内部のストレージに保存されます．

            5. 「連結結果をダウンロード」ボタンを押すと，話者分離したテキストファイルが１つのファイルの連結されて，自分のパソコンに
            ダウンロードできます．


## 2. このページの役割（話者分離）

- このページは、文字起こしテキスト（.txt）を **話者ごとに分離** するための前処理ページです。
- 1つの巨大テキストを一回で整形するのではなく、**複数のファイル（複数 .txt）を順番に処理して保存**し、最後に **つなぎ行を挟んで連結**します。

---

## 3. 入力方式（2通り）

このページは、入力を次の2通りから選べます。

### A) 既存ジョブ（storage より読み込み）
- すでに Storages に存在する job を選びます。
- 選択した job の `transcript/*.txt` を読み込み、順番に話者分離します。

### B) 新規アップロード（drop → 新規job作成 → transcript/ に保存）
- 文字起こしテキスト（.txt）を **複数** アップロードできます。
- アップロード順が、そのまま **処理順** になります。
- **重要：アップロードしただけでは job は作りません。**  
  「📦 アップロードを transcript/ として保存して新規ジョブを作成」ボタンを押した時点で、Storages に新規 job を作成し、`transcript/` に保存します（確定）。

---

## 4. 連続処理（バッチ）の流れ

1. 入力を確定します（既存 job を選択、または新規アップロード→ジョブ作成）。
2. `transcript/*.txt` を順番に処理し、話者分離結果を保存します：
   - `transcript_speaker_separated/`（ファイルごとに保存）
3. それらを「つなぎ行」を挟んで連結し、まとめて保存します：
   - `transcript_speaker_separated_combined/`

> 「つなぎ行」は、どのファイルとどのファイルの間かが分かるように挿入されます。

---

## 5. drop を “連続して” 使える（繰り返し運用）

- 新規アップロード（drop）は **何回でも繰り返し** 行えます。
- たとえば、  
  - 午前の会議 → drop → job作成 → 話者分離  
  - 午後の会議 → drop → job作成 → 話者分離  
  のように、別のアップロードを **別ジョブとして連続運用** できます。
- 同じアップロード内容で画面が再実行（rerun）されても、作成済み job があれば復元して続行できます。

---

## 6. プロンプト構成（lib.prompts）

- `SPEAKER_PREP` を `get_group()` で取得し、プロンプト UI を組み立てます。
- 実行時は `build_prompt()` により、次の 3パートを連結して 1本のプロンプトにします。

1. **必須プロンプト（mandatory）**
2. **プリセット本文（preset）**
3. **追加指示（extra）**

---

## 7. 実行ロジック（バッチ実行）

「② transcript を順番に話者分離（ストレージ保存＋連結）」を押すと：

1. 確定済みの job_dir と transcript_files を取得します。
2. transcript を先頭から順に LLM に投げ、結果を保存します。
3. 途中経過（進捗）を表示しながら最後まで処理します。
4. 最後に連結結果を保存し、ダウンロードも可能です。

※ 基本は 1ファイルずつ順番に処理します（ページ側でリトライは行いません）。

---

## 8. 保存先（ストレージ）

選択された job フォルダ配下に保存されます：

- 入力：`transcript/`
- 個別結果：`transcript_speaker_separated/`
- 連結結果：`transcript_speaker_separated_combined/`
- ログ：`logs/process.log`


"""
        )


###############
#
# 議事録作成
#
###############

# minutes_help_fixed.txt
##############
#
# 議事録作成
#
###############

# ============================================================
# ✅ Minutes Maker: expander を作らない「中身だけ」版
# ============================================================

def render_minutes_maker_help_content() -> None:
    """
    「④ 議事録作成」説明の“中身だけ”を描画する。
    - ここでは st.expander を使わない（ネスト禁止対策）
    """
    st.markdown(
        """
            ## 1. このページの役割

            - 人手で作成された
              **逐語録** を入力として、
              **正式な議事録を生成するページ**です。

            ---

            ## 2. 全体の処理の流れ

            1. 左カラム（サイドバー）で **AIモデル** とを選びます。
            （原則，デフォルトのままで良いです．特に変更する必要はありません．）
            2. 右カラム（メインエリア）の **逐語録入力** から逐語録をアップロードするか、直接貼り付けます。
            （アップロードはパソコンまたはInboxから行うことができます．）
            アップロード後，「読み込み」および「セット」を行ってください．
            3. **プロンプトの設定** では，まず議事録の種類をボタン（ラジオボタン）で選択します．その後「見た目のスタイル」「追記プリセット」を設定します．
            4. **追加指示** では任意のプロンプトを追加することができます．必要のない時は空欄のままで構いません．
            5. 「📝 議事録を生成」ボタンを押して、生成AIを用いて議事録を生成します。
            6. 生成結果が画面に表示され，料金・トークン数，AI使用時間などを確認できます。
            7. **議事録の保存** では，テキストとWord形式で議事録がパソコンに保存できます．

            ---

        """
    )


def render_minutes_maker_help_content_OLD() -> None:
    """
    「④ 議事録作成」説明の“中身だけ”を描画する。
    - ここでは st.expander を使わない（ネスト禁止対策）
    """
    st.markdown(
        """
            ## 1. このページの役割

            - 人手で作成された
              **逐語録** を入力として、
              **正式な議事録を生成するページ**です。

            ---

            ## 2. 全体の処理の流れ

            1. 左カラム（サイドバー）で **AIモデル** とを選びます。
            （原則，デフォルトのままで良いです．特に変更する必要はありません．）
            2. 右カラム（メインエリア）の **逐語録入力** から逐語録をアップロードするか、直接貼り付けます。
            （アップロードはパソコンまたはInboxから行うことができます．）
            アップロード後，「読み込み」および「セット」を行ってください．
            3. **プロンプトの設定** では，まず議事録の種類をボタン（ラジオボタン）で選択します．その後「見た目のスタイル」「追記プリセット」を設定します．
            4. **追加指示** では任意のプロンプトを追加することができます．必要のない時は空欄のままで構いません．
            5. 「📝 議事録を生成」ボタンを押して、生成AIを用いて議事録を生成します。
            6. 生成結果が画面に表示され，料金・トークン数，AI使用時間などを確認できます。
            7. **議事録の保存** では，テキストとWord形式で議事録がパソコンに保存できます．

            ---

            ## 3. プロンプト構造（内容 × 見た目の二段構成）

            このページのプロンプトは、次のような構造になっています。

            ### 3-1. 内容系（MINUTES_MAKER グループ）

            - `MINUTES_MANDATORY_MODES` で  
              「逐語録 / 逐語録のまとめ / 簡易議事録 / 詳細議事録」などの  
              **議事録の種類ごとの必須パート**を定義しています。
            - ラジオボタン「議事録の種類」でモードを選ぶと、
              対応する必須パートが `minutes_mandatory` に自動反映されます。
            - さらに `lib.prompts` の `MINUTES_MAKER` グループから
              - 追記プリセット（内容）
              - その本文（編集可）
              を取得して、**内容面の補足指示**を組み立てます。

            ### 3-2. 見た目スタイル（MINUTES_STYLE グループ）

            - `MINUTES_STYLE` グループには、
              - ベーシックな箇条書き
              - 横線やセクション罫線を多用するスタイル
              - Word 見出しとの相性を重視したスタイル
              など、**見た目（レイアウト）に関する指示**をまとめています。
            - ラジオボタン「見た目のスタイル」でラベルを選ぶと、
              その本文を `style_body = style_group.body_for_label(...)` で取得します。

            ### 3-3. 内容プリセット + 見た目スタイルの合体

            - まず「内容プリセット」（`minutes_preset_text`）をベースにし、
            - そこに

              ```text
              【見た目のスタイル指示】
              （MINUTES_STYLE 側の本文）
              ```

              を後ろに付けたものを `merged_preset` として使います。

            ### 3-4. 最終プロンプトの組み立て

            - `build_prompt()` に渡す 4つの要素は：

              1. `minutes_mandatory`（議事録の種類に応じた必須パート）
              2. `merged_preset`（内容プリセット + 見た目スタイル指示）
              3. `minutes_extra_text`（任意の追加指示）
              4. `src`（整形済みテキスト本体）

            - これらを 1本の長いプロンプトに連結して、LLM に渡します。

            ---

            ## 4. 入力テキスト（右カラム）の扱い

            - ③ページの出力を
              - `.txt` または `.docx` ファイルとしてアップロード
              - あるいはテキストエリアへコピペ
              する想定です。

            ### 4-1. .docx アップロード

            - `python-docx`（`docx` モジュール）が利用可能な場合：
              - ファイルを読み込み、すべての段落を結合して 1つのテキストにします。
            - `python-docx` が無い場合：
              - エラーを表示し、インストール (`pip install python-docx`) を促します。

            ### 4-2. .txt アップロード

            - UTF-8 → CP932 → その他（`errors="ignore"`）の順でデコードを試み、
              読み込んだテキストを `minutes_source_text` に格納します。

            ### 4-3. テキストエリア

            - `minutes_source_text` の中身を初期値として表示し、
              直接編集も可能です。
            - 空のまま「議事録を生成」を押すと「整形済みテキストを入力してください」と警告します。

            ---

            ## 5. モデル設定と温度・トークン上限

            サイドバーで次を指定します。

            - モデル：
              - `gpt-5`, `gpt-5-mini`, `gpt-5-nano`, `gpt-4.1-mini`, `gpt-4.1`
            - 温度（0〜2）：
              - GPT-5 系は `supports_temperature()` により **temperature 固定** とみなし、
                スライダーを無効化 & API に temperature を送信しません。
              - GPT-4.1 系は、1.0と異なる場合のみ `chat_kwargs["temperature"]` に設定します。
            - `max_completion_tokens`：
              - modern の `max_completion_tokens` としてそのまま渡されます。
              - 長めの議事録を一度に生成したい場合は 8,000〜12,000 程度を推奨。

            ---

            ## 6. モデル呼び出し（リトライなし一発実行）

            「📝 議事録を生成」ボタンを押すと：

            1. 上述のプロンプト（combined）を作成。
            2. `call_once()` を通じて `client.chat.completions.create()` を **1回だけ**実行。
               - 引数：
                 - `model`
                 - `messages=[{"role": "user", "content": combined}]`
                 - `max_completion_tokens`
                 - （必要に応じて）`temperature`
            3. `resp.choices[0].message.content`（または `.text`）から議事録本文を取得。
            4. 結果が空でなければ `minutes_final_output` として `session_state` に保存し、
               Markdown で表示します。
            5. `finish_reason == "length"` の場合は、
               「出力上限で切れている可能性あり」として info を表示します。
            6. 結果が空の場合は、レスポンス全体 (`resp.model_dump()`) を表示してデバッグできます。

            ---

            ## 7. トークン・料金の概要

            - `extract_tokens_from_response(resp)` で
              **入力トークン / 出力トークン / 合計トークン** を取得します。
            - `estimate_chat_cost_usd(model, input_tok, output_tok)` で USD の概算コストを計算し、
              サイドバーの `USD/JPY` レートを掛けて **JPY の概算** を出します。
            - これらを小さな表として表示し、
              実行ごとのコストを把握しやすくしています。
            - さらに「🔍 トークン算出の内訳」expander では、
              `debug_usage_snapshot(resp.usage)` により modern usage の中身を確認できます。

            ---

            ## 8. 生成結果の保存（.txt / .docx）

            ### 8-1. テキスト保存（.txt）

            - `minutes_final_output` を UTF-8 でエンコードし、
              「💾 テキストで保存 (.txt)」ボタンからダウンロードできます。

            ### 8-2. Word 保存（.docx）

            - `python-docx` が利用可能な場合：
              - Markdown 的な構造を簡易に解釈して Word スタイルに変換します。
              - 主なルール：
                - 空行 → 空段落
                - `---` / `―――` / `ーーー` → 段落下罫線（pBdr）
                - `# 見出し` → Heading 1
                - `## 見出し` → Heading 2
                - `- 箇条書き` → List Bullet スタイル
                - その他の行 → Normal スタイル（行間 1.2、後ろに少しスペース）
              - フォントはデフォルトで「游ゴシック」、サイズ 11pt（通常）、14pt（見出し1）、12pt（見出し2）を指定。
              - ファイル名は
                - 議事録の種類（mode_label）
                - 現在日時（`YYYYMMDD_HHMM`）
                を含めた `"minutes_{safe_label}_{timestamp}.docx"` で出力します。

            ---

            ## 9. 典型的な運用フロー

            1. ① 文字起こし（連続対応）で音声 → テキスト。
            2. ③ 話者分離・整形で、S1/S2形式などの「読みやすい逐語録」に整形。
            3. この④ページに結果を渡し、
               - 「議事録の種類」を選ぶ（例：簡易議事録）
               - 「見た目のスタイル」を選ぶ（例：Word見出し向け）
            4. 必要なら追加指示（議題の順番、決定事項の書き方など）を記入。
            5. 「議事録を生成」を実行。
            6. 画面で確認し、`.txt` や `.docx` を保存して配布。

            以上が、「④ 議事録作成（整形済みテキスト → 議事録）」ページのロジックと使い方です。
        """
    )



def render_minutes_prompt_spec_help_content() -> None:
    """
    Minutes Maker の「GPT に送るプロンプト原文」説明の“中身だけ”を描画する。
    - ここでは st.expander を使わない（ネスト禁止対策）
    """
    st.markdown(
        """
## Minutes Maker に渡されるプロンプト構造（GPT への最終プロンプト）

このページでは，Minutes Maker から GPT に送られるプロンプト原文を，構成要素ごとにそのままの形で示します。  
実際の呼び出しでは，下記 1〜4 をこの順番で連結したものが 1 本のプロンプトとして送信されます。

1. Minutes Maker 全体共通 mandatory（`MINUTES_GLOBAL_MANDATORY`）
2. モード別 mandatory（逐語録／簡易議事録／詳細議事録のいずれか 1 本）
3. 追記プリセット＋見た目スタイル（`merged_preset`）
4. 追加指示（任意）と入力テキスト（整形済みテキスト）

---

## 1. Minutes Maker 全体共通 mandatory

`MINUTES_GLOBAL_MANDATORY` の原文は次の通りです。

---

### 1-1. MINUTES_GLOBAL_MANDATORY 本文

あなたは日本語の会議議事録（逐語録・簡易議事録・詳細議事録）を作成する専門家です。  
以下の入力テキスト（整形済みテキスト：話者ごとに分離されたテキスト）をもとに，  
指定されたモードに応じた形式の議事録を出力してください。

【共通方針】

- 事実関係を改変してはいけません。推測や補完を行う場合は，「〜と考えられる」「〜と思われる」など不確実性が分かる表現で示してください。
- 日付・時刻・数値は，原則として半角で統一してください（例: 2025-12-08, 10:30, 3 件）。
- 固有名詞（人名・地名・組織名・資料名など）は，入力テキスト中の表記をできるだけ尊重してください。
- 同じ内容を繰り返し説明しないようにし，読み手が「何が議論され，何が決まり，次に何をするのか」を素早く把握できる構成を心がけてください。

【書式・見た目に関する共通ルール】

- 見出しには Markdown の見出し記法（`#`, `##`, `###`）を使用してください（行頭に全角スペースを入れない）。
- 箇条書きは「`- `」（半角ハイフン＋半角スペース）を用いてください。Tab は使わず，必要に応じて半角スペースでインデントしてください。
- 行頭に「*」「●」「■」などの記号を用いた独自の箇条書きは使わないでください。
- 不要な空行は避け，見出しやセクションの区切りの前後など，意味のある箇所にだけ空行を入れてください。

【表の出力に関する共通ルール（重要）】

- 「変更指示一覧」「TODO 一覧」「決定事項一覧」など，表形式の出力が必要な場合は，必ず Markdown の表記法を用いて出力してください。
- 表の構造は次の形式に従ってください（例）：

  | 列1 | 列2 | 列3 |
  | --- | --- | --- |
  | 値1 | 値2 | 値3 |

  ※この例は構造を示すだけであり，実際の列名・列数は指示に応じて変更して構いません。

- 表に関する厳守事項:
  - 各行の先頭と末尾，および列の区切りには半角の縦棒 `|` を使ってください。全角の「｜」は絶対に使わないでください。
  - 行頭に余分な空白（特に全角スペース）を入れないでください。
  - コードブロック（``` で囲む形式）は使わないでください。表そのものだけを出力してください。
  - セル内で改行しないでください。複数文が必要な場合でも 1 セル内に 1 つの段落としてまとめてください。
  - セル内に `|` を含める必要がある場合は，必ず `\\|` とエスケープしてください。
  - 空欄セルが必要なときは完全な空文字ではなく，半角スペース 1 つを入れておいてください。

- 「表形式だけを出力してください」「表のみを返してください」などと明示された場合は，上記の Markdown 表以外のテキスト（説明文・見出し・前置きなど）を書いてはいけません。  
  そうでない場合は，議事録本文のなかに必要に応じて表を挿入して構いません。

これらの共通ルールは，逐語録・簡易議事録・詳細議事録のいずれのモードでも必ず守ってください。

---

（以下、入力した逐語録が続く）
        """
    )


def render_minutes_help_expander_tabs() -> None:
    """
    ✅ 公開API（wrapperなし）
    - 1つの expander の中に tabs を作り、
      各タブで「中身だけ」を描画する
    """
    with st.expander("📌 説明（クリックで展開）", expanded=False):
        tab_usage, tab_prompt = st.tabs(
            ["④ 議事録作成（使い方）", "🧾 GPTに送るプロンプト（原文）"]
        )

        with tab_usage:
            render_minutes_maker_help_content()

        with tab_prompt:
            render_minutes_prompt_spec_help_content()


            
# ============================================================
# 重複箇所検出（ストレージ対応）
# ============================================================

def render_overlap_detect_storage_expander() -> None:
    """
    「重複箇所検出（ストレージ対応）」ページのロジックと使い方を expander で描画します。
    pages/30_重複箇所検出_storage対応.py などから呼び出して使います。
    """
    with st.expander("ℹ️ 重複箇所検出（ストレージ対応）のしくみと使い方（クリックで展開）", expanded=False):
        st.markdown(
            """
            ## 1. このページの役割

            - 連結文字起こし（複数ファイルをつないだテキスト）に含まれる  
              **つなぎ目付近の「重複箇所」** を検出して、編集しやすいように **目印（タグ）を挿入**します。
            - このツールは **自動削除は行いません**。  
              誤削除のリスクを避けるため、重複の開始位置に `-----ここから重複-----` を付けて、
              **人が確認して削除・整形できる状態**にします。

            ---

            ## 2. 対象ファイル（どこを見ている？）

            ログインユーザーの Storages 配下から、次の位置を探索して候補を一覧します。

            - `Storages/<user>/minutes_app/<YYYY-MM-DD>/job_*/transcript_speaker_separated_combined/*.txt`

            画面のラベルには、日付 / job_id / ファイル名 / created_at（あれば）が含まれます。

            ---

            ## 3. 全体の処理の流れ

            1. 対象（combined txt）を 1つ選択します（radio）。
            2. テキストを **つなぎ目マーカー**でセグメント分割します。  
               `----- ここがつなぎ目です（...）-----` の行が「区切り」として使われます。
            3. 各つなぎ目について、後半セグメントの冒頭から **キー文字列**（先頭数文）を作成します。
            4. 前半セグメント末尾から一致箇所を探し、見つかった場合は  
               **前半側にだけ** `-----ここから重複-----` を挿入します。
            5. 全つなぎ目の処理が終わったら、結果テキストと検出ログをストレージに保存します。
            6. 画面上に「つなぎ目ごとの検出ログ」を一覧表示し、必要に応じて詳細も確認できます。

            ---

            ## 4. 検出ロジック（ざっくり）

            - 後半セグメント先頭から「キー文」を作り、前半末尾（一定文字数）と照合します。
            - 照合時は、空白・改行・句読点などを除去して **正規化**した文字列で比較します。
            - 後半先頭に `S1:` / `[s12]:` / `司会:` のような **話者ラベルが付いている場合は除去**してから
              キー文を作ります（先頭ノイズ対策）。
            - さらに、先頭の1文字をずらした候補も試し、先頭が欠けていても拾えるようにしています。

            ※「重複の完全削除」ではなく、編集の起点となる **開始位置のマーキング** が目的です。

            ---

            ## 5. サイドバーのパラメータ

            ### 🔹 一致とみなす最低文字数（MIN_MATCH_SIZE）
            - 小さいほど検出がゆるくなり、短い一致も拾いやすくなります（誤検出も増えやすい）。
            - 大きいほど検出が厳しくなり、確実性が上がります（見逃しも増えやすい）。
            - 迷ったらデフォルト値から始め、結果を見て調整してください。

            ### 🔹 autojunk（SequenceMatcher 自動ジャンク判定）
            - **ON（デフォルト）**：一般的に安定・高速です。
            - **OFF**：短い一致も拾いやすくなる一方、少し遅くなることがあります。  
              「短い重複が多くて見逃す」場合に OFF を試してください。

            ---

            ## 6. 出力（どこに保存される？）

            実行すると、選択した job フォルダの下に `transcript_marked/` を作成し、そこへ保存します。

            - 結果テキスト：`transcript_marked/<元ファイル名>_marked.txt`
            - 検出ログ（JSON）：`transcript_marked/<元ファイル名>_detect_log.json`

            さらに、job の `logs/process.log` にも最小限の記録（入力・出力・パラメータ）を追記します。

            ---

            ## 7. 画面に表示されるログの見方

            ### 7-1. 「つなぎ目ごとの検出ログ（概要）」
            - 各つなぎ目について
              - 検出結果（検出 / 見つからず）
              - 前半内の開始位置
              - 一致文字数
              を一覧で確認できます。

            ### 7-2. 「head_phrase / matched_phrase 詳細」
            - `head_phrase`：後半冒頭から作ったキー文（ベース）
            - `shifted_phrases`：先頭ずらしの候補（先頭ノイズ対策）
            - `matched_phrase`：実際に一致に使われた候補

            ---

            ## 8. 注意点（重要）

            - つなぎ目マーカーが無いテキストは、セグメント分割できないため検出ログが空になります。
            - Word 変換や後処理で改行・句読点が大きく変わると一致が取りにくくなります。  
              その場合は MIN_MATCH_SIZE を下げる／autojunk を OFF にする、を試してください。
            - このツールは「自動削除」ではなく「マーキング」です。  
              最終的な削除・整形は、結果を見ながら人が判断してください。
            """
        )


